    ...
    <div className="rounded-2xl border border-[var(--color-sand)] bg-white dark:bg-[#242424] p-5 shadow">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-lg font-semibold text-slate-900 dark:text-[#F5F1EB] tabular-nums font-data">
        ...
        <div className="flex h-2 rounded-full overflow-hidden bg-slate-100 dark:bg-[#3A3A3A]">
          ...
          />
          <div
            className="bg-[var(--color-clay)] transition-all duration-300"
            style={{ width: `${carbsPct}%` }}
          />
          <div
            className="bg-[var(--color-terracotta)] transition-all duration-300"
            style={{ width: `${fatPct}%` }}
          />
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 text-center">
          {(
            [
              ...
              { labelKey: "Carbs" as const, value: macros.carbs, color: "text-[var(--color-clay)]", bg: "bg-[var(--color-clay-light)]" },
              { labelKey: "Fat" as const, value: macros.fat, color: "text-[var(--color-terracotta)]", bg: "bg-[#FBF0EB]" },
            ]
          ).map(({ labelKey, value, color, bg }) => (
            <div key={labelKey} className={cn("rounded-lg p-2", bg)}>
              <p className={cn("text-base font-semibold tabular-nums font-data", color)}>
        ...
        {hasData && (
          <div className="flex items-center justify-center gap-3 pt-1">
            {[
              ...
              { label: "C", color: "bg-[var(--color-clay)]", pct: carbsPct },
              { label: "F", color: "bg-[var(--color-terracotta)]", pct: fatPct },
            ].map(({ label, color, pct }) => (
  ...
  return (
    ...
    <div className="flex flex-col lg:flex-row gap-6">
      ...
      <div className="flex-1 min-w-0 space-y-6">
        {/* Basic info */}
        <div className="rounded-2xl border border-[var(--color-sand)] bg-white dark:bg-[#242424] p-6 shadow space-y-5">
        ...
        <div className="rounded-2xl border border-[var(--color-sand)] bg-white dark:bg-[#242424] p-6 shadow">
            ...
            <div className="flex items-center gap-2 mb-4">
              ...
              {saltIngredient && !selectedIds.has(saltIngredient.id) && (
                <button
                  ...
                  className="inline-flex items-center gap-1 rounded-full border border-[var(--color-sand)] bg-slate-50 dark:bg-[#2A2A2A] px-2.5 py-0.5 text-xs font-medium text-slate-600 dark:text-[#A0998E] hover:bg-slate-100 dark:hover:bg-[#333] transition-colors"
                >
          ...
          {rows.length === 0 ? (
            <div className="text-center py-8 text-slate-400 dark:text-[#6A6460] text-sm border border-dashed border-[var(--color-sand)] rounded-lg">
              {t("No ingredients yet", lang)}
            </div>
          ) : (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-400 dark:text-[#6A6460] tabular-nums font-data">
                          {row.unit === "pinch" ? (
                            <span className="text-slate-400 dark:text-[#6A6460]">{t("pinch", lang)} · {t("Calories", lang)}: 0</span>
                          ) : (
                            <>
                              {fmtMacro(macros.calories)} kcal ·{" "}
                              <span className="text-[var(--color-olive)]">{fmtMacro(macros.protein)}g P</span> ·{" "}
                              <span className="text-[var(--color-clay)]">{fmtMacro(macros.carbs)}g C</span> ·{" "}
                              <span className="text-[var(--color-terracotta)]">{fmtMacro(macros.fat)}g F</span>
                            </>
                          )}
                        </p>
      ...
      <div className="lg:w-64 lg:shrink-0">
        <div className="lg:sticky lg:top-8">
          ...
          {isEditing && (
            <div className="mt-3 rounded-2xl border border-[var(--color-sand)] bg-white dark:bg-[#242424] p-4 shadow-sm">
              <p className="text-xs text-slate-400 dark:text-[#6A6460] mb-2">{t("Status", lang)}</p>
              <Badge
                className={cn(
                  "text-xs font-medium",
                  initialData.status === "published"
                    ? "bg-[var(--color-olive)] text-white border-[#6A7B5F]"
    ...
    <div className="sticky bottom-0 z-10 bg-white dark:bg-[#1A1A1A] border-t border-[var(--color-sand)] mt-6 -mx-6 sm:-mx-8 px-6 sm:px-8 py-4 flex items-center gap-3">
      <Button
        type="button"
        variant="outline"
